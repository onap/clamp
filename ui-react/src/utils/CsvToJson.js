...
export default function CsvToJson(rawCsvData, delimiter, internalDelimiter, csvHeaderNames, jsonKeyNames, mandatory) {
   ...
   if (checkLength !== jsonKeyNames.length || checkLength !== mandatory.length) {
	...
	if (checkLength < 1) {
		result.errorMesages = 'interanl error: csvHeaderNames, jsonKeyNames, and mandatory arrays have no entries';
		return result;
	}
	...
	for (let i = 0; i < dictElems.length; i++) {
		...
		if (oneRow === '') {
			...
			result.errorMessages += '\nRow #' + i + ' is too short to be valid.';
			break;
	...
	if (headers.length < jsonKeyNames.length) {
		result.errorMessages += 'Not enough columns in row 1 header. Required row header contains at least: \n' + printDictKeys;
		return result;
	}
