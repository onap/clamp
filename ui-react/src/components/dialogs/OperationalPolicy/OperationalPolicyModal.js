...
export default class OperationalPolicyModal extends React.Component {
	...
	renderJsonEditor() {
		...
		var schema_json = this.state.loopCache.getOperationalPolicyJsonRepresentation();
		...
		/*Object.keys(operationalPoliciesData).forEach(opPolicyKey => {
			if (Object.keys(operationalPoliciesData[opPolicyKey].configurationsJson).length === 0) {
				operationalPoliciesData[opPolicyKey].configurationsJson.operational_policy.controlLoopName=this.state.loopCache.getLoopName();
			}
		})*/
