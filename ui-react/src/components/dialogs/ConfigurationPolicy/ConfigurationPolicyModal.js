...
export default class ConfigurationPolicyModal extends React.Component {
    ...
    getMicroserviceArray(editorArray) {
    	...
    	for(var item in editorArray) {
    		...
    		if(this.state.hidePolicyTypeDropdown && this.state.loopCache.updateMicroServiceProperties(this.state.componentName, editorArray[item])) {
				//hidePolicyTypeDropdown is true only when there are more than one policy variants
			...
			} else if(this.state.loopCache.updateMicroServicePropertiesByPolicyModelType(this.state.chosenPolicyModelVariant.value, editorArray[item])){
				existingMsProperty = true;
    		...
    		//existingMsProperty is false then it is a new microservice config policy
	...
	setMicroServiceProps(microServicePolicyArray, policyTypeChangeEvent) {
		LoopService.setMicroServiceProperties(this.state.loopCache.getLoopName(), microServicePolicyArray).then(resp => {
	...
	getPolicyModelVariants() {
		if (typeof this.state.componentName === "undefined" || this.state.componentName === null) {
			...
			if(policyVariants !== null) {
				var policyVariantOptions = policyVariants.map((variant) => { return { label: variant.policyAcronym, value: variant.policyModelType } });
	...
	configureJsonEditor(toscaModel) {
		...
		if(editorData === null && this.state.chosenPolicyModelVariant !== null && this.state.chosenPolicyModelVariant.value !== null) {
			...
			editorData = this.state.loopCache.getMicroServicePropertiesForModelType(this.state.chosenPolicyModelVariant.value, this.state.chosenPolicyModelVariant.label);
		}
