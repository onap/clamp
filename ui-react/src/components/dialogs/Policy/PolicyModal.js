...
export default class PolicyModal extends React.Component {
	...
	renderJsonEditor() {
	    ...
	    if (this.state.policyInstanceType === 'MICRO-SERVICE-POLICY') {
            ...
            if((!toscaModel) &&  this.state.chosenPolicyModelVariant !== null){
            	if((!editorData) && this.state.chosenPolicyModelVariant.value !== null) {
				...
				PolicyToscaService.getPolicyModelTypeJsonSchema(this.state.chosenPolicyModelVariant.value).then(resp => {
					...
					this.configureJsonEditor(toscaModel, editorData);
				});
			} else {
			    this.configureJsonEditor(toscaModel, editorData);
			}
	...
	configureJsonEditor(toscaModel, editorData) {
