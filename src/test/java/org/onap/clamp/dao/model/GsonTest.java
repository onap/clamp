...
public class GsonTest {
    ...
    public void DecodeEncodeTest() {
        ...
        µService.setUsedBy(new HashSet<Loop>(Arrays.asList(loopTest)));
        µService.setName("ConfigPolicyTest");
        loopTest.setMicroServicePolicies(new HashSet<MicroServicePolicy>(Arrays.asList(µService)));
        ...
        Loop loopInDb = loopRepository.save(loop);
        assertNotNull(loopInDb);
        assertTrue(loopRepository.findById(loopInDb.getId()).get().getName().equals("ClosedLoopTest"));
    }
}
