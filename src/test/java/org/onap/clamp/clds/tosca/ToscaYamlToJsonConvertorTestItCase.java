...
public class ToscaYamlToJsonConvertorTestItCase {
    ...
    @Test
    @Transactional
    public final void testToscaWithDateTime() throws IOException {
        setupDictionary();
        setupDictionaryWithDatetime();
        String toscaModelYaml =
            ResourceFileUtil.getResourceAsString("tosca/tosca_metadata_clamp_possible_values.yaml");
        ...
        System.out.println("Json Schema with Date time" + parsedJsonSchema);

        assertNotNull(parsedJsonSchema);
        JSONAssert.assertEquals(
            ResourceFileUtil.getResourceAsString("tosca/tosca_with_date_time_json_schema.json"),
            parsedJsonSchema, true);

    }

    private void setupDictionaryWithDatetime() {
