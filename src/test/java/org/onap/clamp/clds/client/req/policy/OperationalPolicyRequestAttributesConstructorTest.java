...
public class OperationalPolicyRequestAttributesConstructorTest {
    ...
    private OperationalPolicyRequestAttributesConstructor operationalPolicyRequestAttributesConstructor =
            new OperationalPolicyRequestAttributesConstructor(operationalPolicyYamlFormatter);
    ...
    public void shouldFormatRequestAttributes() throws IOException, BuilderException {
        ...
        Assertions.assertThat(requestAttributes.get(AttributeType.MATCHING))
                .contains(Assertions.entry(OperationalPolicyRequestAttributesConstructor.CONTROLLER, "amsterdam"));
        ...
        Assertions.assertThat(ruleParameters).containsExactly(
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.MAX_RETRIES, "3"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.TEMPLATE_NAME, "ClosedLoopControlName"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.NOTIFICATION_TOPIC, "POLICY-CL-MGT"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.RECIPE_TOPIC, "APPC"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.RECIPE, "healthCheck"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.RESOURCE_ID, "cdb69724-57d5-4a22-b96c-4c345150fd0e"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.RETRY_TIME_LIMIT, "180"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.CLOSED_LOOP_CONTROL_NAME, CONTROL_NAME + "_1")
        );
    }
    ...
    public void shouldFormatRequestAttributesWithProperControlLoopYaml() throws IOException, BuilderException {
        ...
        //when
        Map<AttributeType, Map<String, String>> requestAttributes = operationalPolicyRequestAttributesConstructor
                .formatAttributes(mockClampProperties, modelProperties,
                        "789875c1-e788-432f-9a76-eac8ed889734", policyChain);

        //then
        Assertions.assertThat(requestAttributes).containsKeys(AttributeType.MATCHING, AttributeType.RULE);
        Assertions.assertThat(requestAttributes.get(AttributeType.MATCHING)).contains(Assertions.entry("controller", "amsterdam"));

        Map<String, String> ruleParameters = requestAttributes.get(AttributeType.RULE);
        Assertions.assertThat(ruleParameters).contains(
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.OPERATION_TOPIC, "APPP-CL"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.TEMPLATE_NAME, "ClosedLoopControlName"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.NOTIFICATION_TOPIC, "POLICY-CL-MGT"),
                Assertions.entry(OperationalPolicyRequestAttributesConstructor.CLOSED_LOOP_CONTROL_NAME, CONTROL_NAME + "_1")
        );
