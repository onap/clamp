...
public class LoopControllerTestItCase {
    ...
    public void testUpdateOperationalPolicies() {
        String policyStr = "[{\"name\":\"OPERATIONAL_CLholmes31_v1_0_vFW_PG_T10_k8s-holmes-rules\"," +
        ...
        JsonElement ele = parser.parse(policyStr);
        JsonArray arr = ele.getAsJsonArray();
        Loop l = loopController.updateOperationalPolicies(EXAMPLE_LOOP_NAME, arr);
        assertThat(l.getOperationalPolicies()).hasSize(1);
        Set<OperationalPolicy> opSet = l.getOperationalPolicies();
    ...
    public void testUpdateGlobalProperties() {
        String p = "{\"dcaeDeployParameters\":{\"aaiEnrichmentHost\":\"aai.onap.svc.cluster.local\"," +
        ...
        JsonParser parser = new JsonParser();
        JsonElement ele = parser.parse(p);
        JsonObject obj = ele.getAsJsonObject();
        loopController.updateGlobalPropertiesJson(EXAMPLE_LOOP_NAME, obj);
        Loop l = loopController.getLoop(EXAMPLE_LOOP_NAME);
        JsonObject globalPropertiesJson = l.getGlobalPropertiesJson();
    ...
    public void testUpdateMicroservicePolicy() {
        MicroServicePolicy p = new MicroServicePolicy("policyName", "",
                                                      "tosca_definitions_version: tosca_simple_yaml_1_0_0", false,
                                                      JsonUtils.GSON.fromJson(EXAMPLE_JSON, JsonObject.class), null);
        loopController.updateMicroservicePolicy(EXAMPLE_LOOP_NAME, p);
    ...
    public void testGetSvgRepresentation() {
       String s =  loopController.getSvgRepresentation(EXAMPLE_LOOP_NAME);
       assertThat(s).isEqualTo("representation");
    }
}