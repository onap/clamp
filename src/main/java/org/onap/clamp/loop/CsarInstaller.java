...
public class CsarInstaller {
    ...
    public void installTheCsar(CsarHandler csar) throws SdcArtifactInstallerException, InterruptedException {
        ...
        Service service = installTheService(csar);
        installTheLoop(csar, service);
        logger.info("Successfully installed the CSAR " + csar.getFilePath());
    }

    @Transactional(propagation = Propagation.REQUIRED)
    public void installTheLoop(CsarHandler csar, Service service) throws SdcArtifactInstallerException, InterruptedException {
        try {
            logger.info("Installing the Loops");
            if(csar.getMapOfBlueprints().entrySet().size() == 0) {
                logger.info("No Loops detected in the csar");
                return;
            }
            for (Entry<String, BlueprintArtifact> blueprint : csar.getMapOfBlueprints().entrySet()) {
    ...
    public Service installTheService(CsarHandler csar) {
        ...
        Service modelService = new Service(serviceDetails, resourcesProp, csar.getSdcNotification().getServiceVersion());
