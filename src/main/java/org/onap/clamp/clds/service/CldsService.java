...
//        cldsInfo.setPermissionUpdateTemplate(isAuthorizedNoException(permissionUpdateTemplate));
     ...
     * @throws GeneralSecurityException In case of issues when decrypting the password
     * @throws DecoderException In case of issues with the Hex String decoding
     */
    @PUT
    ...
    public Response putModelAndProcessAction(@PathParam("action") String action,
        ...
        try {
            ...
            if (!isTest && (actionCd.equalsIgnoreCase(CldsEvent.ACTION_SUBMIT)
                ...
                if (retrievedModel.getTemplateName()
                    .startsWith(CsarInstallerImpl.TEMPLATE_NAME_PREFIX)) {
                    // SDC artifact case
                    logger.info(
                        "Skipping DCAE inventory call as closed loop has been created from SDC notification");
                    DcaeEvent dcaeEvent = new DcaeEvent();
                    dcaeEvent.setArtifactName(retrievedModel.getControlName() + ".yml");
                    dcaeEvent.setEvent(DcaeEvent.EVENT_DISTRIBUTION);
                    CldsEvent.insEvent(cldsDao, dcaeEvent.getControlName(), userId,
            ...
            LoggingUtils.setTimeContext(startTime, new Date());
            LoggingUtils
                .setResponseContext("0", "Process model action success", this.getClass().getName());
        ...
        if (errorCase) {
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(retrievedModel)
                .build();
        }
    ...
    public String postDcaeEvent(@QueryParam("test") String test, DcaeEvent dcaeEvent) {
        ...
        if (cldsPermissionTypeClEvent != null && cldsPermissionTypeClEvent.length() > 0) {
            SecureServicePermission permissionEvent = SecureServicePermission
                .create(cldsPermissionTypeClEvent,
        ...
        String msgInfo =
            "event=" + dcaeEvent.getEvent() + " serviceUUID=" + dcaeEvent.getServiceUUID()
                + " resourceUUID=" + dcaeEvent.getResourceUUID() + " artifactName=" + dcaeEvent
                .getArtifactName()
     ...
     *
     * @throws GeneralSecurityException In case of issue when decryting the SDC password
     * @throws DecoderException In case of issues with the decoding of the Hex String
     */
    @GET
    @Path("/sdc/services")
    @Produces(MediaType.APPLICATION_JSON)
     ...
     *
     * @throws GeneralSecurityException In case of issues with the decryting the encrypted password
     * @throws DecoderException In case of issues with the decoding of the Hex String
     * @throws IOException In case of issue to convert CldsServiceCache to InputStream
     */
    @GET
    ...
    public String getSdcPropertiesByServiceUUIDForRefresh(
        ...
        Date startTime = new Date();
        LoggingUtils
            .setRequestContext("CldsService: GET sdc properties by uuid", getPrincipalName());
        ...
        if (sdcCatalogServices.isCldsSdcCacheDataExpired(cldsServiceData)) {
            cldsServiceData = sdcCatalogServices
                .getCldsServiceDataWithAlarmConditions(serviceInvariantUUID);
        ...
        // audit log
        LoggingUtils.setTimeContext(startTime, new Date());
        LoggingUtils.setResponseContext("0", "Get sdc properties by uuid success",
    ...
    public boolean isAuthorizedForVf(String vfInvariantUuid) {
        if (cldsPermissionTypeFilterVf != null && !cldsPermissionTypeFilterVf.isEmpty()) {
            SecureServicePermission permission = SecureServicePermission
                .create(cldsPermissionTypeFilterVf,
    ...
    private String createUiServiceFormatJson(String responseStr) throws IOException {
        ...
        List<SdcServiceInfo> rawList = objectMapper.readValue(responseStr,
            objectMapper.getTypeFactory()
                .constructCollectionType(List.class, SdcServiceInfo.class));
        ...
        if (rawList != null && !rawList.isEmpty()) {
            List<SdcServiceInfo> cldsSdcServiceInfoList = sdcCatalogServices
                .removeDuplicateServices(rawList);
    ...
    private String createPropertiesObjectByUUID(String cldsResponseStr) throws IOException {
        ...
        SdcServiceDetail cldsSdcServiceDetail = mapper
            .readValue(cldsResponseStr, SdcServiceDetail.class);
        ...
        if (cldsSdcServiceDetail != null && cldsSdcServiceDetail.getUuid() != null) {
             ...
             */
            ObjectNode emptyvfcobjectNode = createByVFCObjectNode(
    ...
    private void createVfObjectNode(ObjectNode vfObjectNode2,
        ...
        if (cldsSdcResourceList != null && !cldsSdcResourceList.isEmpty()) {
            for (SdcResource cldsSdcResource : cldsSdcResourceList) {
                if (cldsSdcResource != null && "VF"
                    .equalsIgnoreCase(cldsSdcResource.getResoucreType())) {
                    vfNode
                        .put(cldsSdcResource.getResourceUUID(), cldsSdcResource.getResourceName());
                }
            }
        }
        ...
        try {
            ...
            logger.error(
                "Unable to load ui.location.default JSON in clds-references.properties properly",
                e);
        ...
        try {
            ...
            throw new CldsConfigException(
                "Unable to load ui.alarm.default JSON in clds-references.properties properly",
                e);
        }
