...
public class Painter {
    ...
    DocumentBuilder doPaint(String collector, List<String> microServices, String policy) {
        ...
        ib.cicrle("start-circle", SLIM_LINE)
            .arrow()
            .rect(collector, RectTypes.COLECTOR, collector);

        for(String ms : microServices) {
            ib.arrow().rect(ms, RectTypes.MICROSERVICE, ms);
        }

        ib.arrow()
            .rect(policy, RectTypes.POLICY, policy)
            .arrow()
            .cicrle("stop-circle", THICK_LINE);
