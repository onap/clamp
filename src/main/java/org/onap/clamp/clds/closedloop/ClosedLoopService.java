...
package org.onap.clamp.clds.closedloop;
...
public class ClosedLoopService {
    ...
    Loop createEmptyClosedLoop(String name, String blueprint, String svg) {
        Loop loop = new Loop(name, blueprint, svg);
    ...
    Loop updateOperationalPolicies(String loopName, List<OperationalPolicy> newOperationalPolicies) {
        ...
        Set<OperationalPolicy> oldPolicies = loop.getOperationalPolicies();
        Set<OperationalPolicy> newPolicies = operationalPolicyService
            .updatePolicies(loop, newOperationalPolicies);

        operationalPolicyService.clearOldPolicies(newPolicies, oldPolicies);

        loop.setOperationalPolicies(newPolicies);
        return loopsRepository.save(loop);
    }

    public Loop updateMicroservicePolicies(String loopName, List<MicroServicePolicy> newMicroservicePolicies) {
        Loop loop = findClosedLoopByName(loopName);
        Set<MicroServicePolicy> oldPolicies = loop.getMicroServicePolicies();
        Set<MicroServicePolicy> newPolicies = microservicePolicyService
            .updatePolicies(loop, newMicroservicePolicies);
        microservicePolicyService.clearOldPolicies(newPolicies, oldPolicies);
