...
public class ChainGenerator {
    ...
    private boolean preValidate(Set<MicroService> input) {
        List<MicroService> noInputs = input.stream().filter(ms -> "".equals(ms.getInputFrom())).collect(Collectors.toList());
        return noInputs.size() == 1;
    }

    private boolean postValidate(LinkedList<MicroService> theList) {
        for (int i = 1; i < theList.size() - 1; i++) {
            MicroService prev = theList.get(i-1);
            MicroService current = theList.get(i);
