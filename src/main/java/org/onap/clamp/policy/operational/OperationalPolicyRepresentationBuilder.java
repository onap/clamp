...
public class OperationalPolicyRepresentationBuilder {
    ...
    public static JsonObject generateJsonRepresentation(Loop loop) throws JsonSyntaxException, IOException {
        ...
        jsonSchema.get("schema").getAsJsonObject().get("items").getAsJsonObject().get("properties").getAsJsonObject()
                ...
                .getAsJsonObject().get("oneOf").getAsJsonArray().addAll(createOneOfArray(loop));
        return jsonSchema;
    }
    ...
    private static JsonArray createVnfOneOfStructure(Loop loop) {
        JsonArray vnfSchemaArray = new JsonArray();
        JsonObject modelVnfs = loop.getModelPropertiesJson().get("resourceDetails").getAsJsonObject().get("VF")
                .getAsJsonObject();
    ...
    private static JsonArray createVfModuleStructure(Loop loop) {
        JsonArray vfModuleOneOfSchemaArray = new JsonArray();
        JsonObject modelVfModules = loop.getModelPropertiesJson().get("resourceDetails").getAsJsonObject()
                .get("VFModule").getAsJsonObject();
    ...
    private static JsonArray createOneOfArray(Loop loop) {
        JsonArray targetOneOfStructure = new JsonArray();
        targetOneOfStructure.addAll(createVnfOneOfStructure(loop));
        targetOneOfStructure.addAll(createVfModuleStructure(loop));
