...
public class OperationalPolicyRepresentationBuilder {
        ...
        for (JsonElement actor : actors) {
            if (actor.getAsJsonObject().get("title").toString().equals("CDS")) {
                actor.getAsJsonObject().get("properties").getAsJsonObject().get("type").getAsJsonObject()
                        .get("anyOf").getAsJsonArray().addAll(createAnyOfArrayForCdsRecipe(modelJson.getResourceDetails()));
            }
        }
    ...
    private static JsonArray createAnyOfArrayForCdsRecipe(JsonObject resourceDetails) {
        ...
        anyOfStructure.add(createAnyOfCdsRecipe(resourceDetails.getAsJsonObject("VF")));
        anyOfStructure.add(createAnyOfCdsRecipe(resourceDetails.getAsJsonObject("PNF")));
    ...
    private static String createDefaultStringForPayload(JsonObject workFlow, JsonObject controllerProperties) {
        ...
        String data = workFlow.getAsJsonObject("inputs").getAsString();
        StringBuilder builder = new StringBuilder("artifact_name : ").append(artifactName).append("\n")
