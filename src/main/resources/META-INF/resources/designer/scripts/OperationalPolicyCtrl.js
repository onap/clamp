...
//	    }
		    ...
		    if (!loadingId) {
			    ...
			    for (var i = 1; i <= greatestIdNum(); i++) {
				    if ($("#formId" + i).length > 0) {
					    ...
					    for (k in policy_ids) {
						    if (($("#formId" + i + " #id").val() !== policy_ids[k].toString())
	    ...
	    function savePolicyLocally(policyId) {
		    ...
		    for (var i = 0; i < properties.length; i++) {
			    policiesArray.push(serializeElement($(properties[i])));
		    }
	    ...
	    function configureComponents(allPolicies, policyId) {
	    	...
	    	$.each(allPolicies[policyId]['policies'],function() {
	    		...
	    		$.each ($('.saveProps').find('.form-control'),function() {
	    			$("#formId" + formNum + " #"+this.id).val(allPolicies[policyId]['policies'][formNum-1][this.id]);
		    	});
	    	});
	    }
	    	      ...
	    	      if (parentPolicyConditions_values) {
	    	        for (key in parentPolicyConditions_values) {
	    	          if ($("#parentPolicyConditions").length > 0) {
	    	            $("#parentPolicyConditions").append("<option value=\"" + key + "\">" + parentPolicyConditions_values[key] + "</option>")
	    	          }
	    	        }
	    	        //$("#parentPolicyConditions").multiselect("choose");
	    	      }
	    	      ...
	    	      if (timeUnitsGuard_values) {
	    	        for (key in timeUnitsGuard_values) {
	    	          if ($("#timeUnitsGuard").length > 0) {
	    	            $("#timeUnitsGuard").append("<option value=\"" + key + "\">" + timeUnitsGuard_values[key] + "</option>")
	    	          }
	    	        }
